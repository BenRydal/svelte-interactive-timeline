 ...
 * Exports both a factory function (for multiple instances) and a default singleton.
 */
 ...
 * Use this factory when you need multiple independent timelines.
 */
export function createTimelineStore() {
	const { subscribe, update } = writable<TimelineState>(initialState);
	...
	return {
		...
		applyZoomSelection() {
			update((s) => {
				const cleared = { ...s, zoomSelectionStart: null, zoomSelectionEnd: null, isDragging: null } as TimelineState;
				if (s.zoomSelectionStart === null || s.zoomSelectionEnd === null) return cleared;
				...
				if (end - start <= 0.5) return cleared;
				return {
					...cleared,
					viewStart: clamp(start, s.dataStart, s.dataEnd),
					viewEnd: clamp(end, s.dataStart, s.dataEnd)
...
/** Default singleton store for simple use cases */
export const timelineStore = createTimelineStore();

/** Current view duration */
export const viewDuration = derived(timelineStore, ($t) => $t.viewEnd - $t.viewStart);

/** Total data duration */
export const dataDuration = derived(timelineStore, ($t) => $t.dataEnd - $t.dataStart);
